...
canFunction :: (MonadNameGen m) => Function -> m Function
canFunction m
    = do body' <- mapM canStm (body m)
         return Function { funcname = funcname m,
                           nparams = nparams m,
                           body = concatMap asList body',
...
canPrg p
    = do mm <- mapM canFunction (functions p)
         return Prg { functions = mm }
